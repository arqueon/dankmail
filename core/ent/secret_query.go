...
func (_q *SecretQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Secret, error) {
	...
	if query := _q.withAccount; query != nil {
		if err := _q.loadAccount(ctx, query, nodes, nil,
			func(n *Secret, e *Account) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SecretQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Secret, init func(*Secret), assign func(*Secret, *Account)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Secret)
	for i := range nodes {
		if nodes[i].account_secrets == nil {
			continue
		}
		fk := *nodes[i].account_secrets
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_secrets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
