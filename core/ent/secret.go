...
func (*Secret) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case secret.ForeignKeys[0]: // account_secrets
	...
	for i := range columns {
		...
		case secret.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field account_secrets", values[i])
			} else if value.Valid {
				_m.account_secrets = new(uuid.UUID)
				*_m.account_secrets = *value.S.(*uuid.UUID)
			}
		default:
...
func (_m *Secret) QueryAccount() *AccountQuery {
	return NewSecretClient(_m.config).QueryAccount(_m)
}
