...
func (_q *AccountQuery) QueryThreads() *ThreadQuery {
...
// QueryPendingOps chains the current query on the "pending_ops" edge.
func (_q *AccountQuery) QueryPendingOps() *PendingOpQuery {
	query := (&PendingOpClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryNotifyRules chains the current query on the "notify_rules" edge.
func (_q *AccountQuery) QueryNotifyRules() *NotifyRuleQuery {
	query := (&NotifyRuleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *AccountQuery) WithThreads(opts ...func(*ThreadQuery)) *AccountQuery {
	query := (&ThreadClient{config: _q.config}).Query()
...
// WithPendingOps tells the query-builder to eager-load the nodes that are connected to
// the "pending_ops" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithPendingOps(opts ...func(*PendingOpQuery)) *AccountQuery {
	query := (&PendingOpClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPendingOps = query
	return _q
}

// WithNotifyRules tells the query-builder to eager-load the nodes that are connected to
// the "notify_rules" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithNotifyRules(opts ...func(*NotifyRuleQuery)) *AccountQuery {
	query := (&NotifyRuleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withThreads; query != nil {
		if err := _q.loadThreads(ctx, query, nodes,
			func(n *Account) { n.Edges.Threads = []*Thread{} },
			func(n *Account, e *Thread) { n.Edges.Threads = append(n.Edges.Threads, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPendingOps; query != nil {
		if err := _q.loadPendingOps(ctx, query, nodes,
			func(n *Account) { n.Edges.PendingOps = []*PendingOp{} },
			func(n *Account, e *PendingOp) { n.Edges.PendingOps = append(n.Edges.PendingOps, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withNotifyRules; query != nil {
...
func (_q *AccountQuery) loadThreads(ctx context.Context, query *ThreadQuery, nodes []*Account, init func(*Account), assign func(*Account, *Thread)) error {
	...
	query.Where(predicate.Thread(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.ThreadsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.account_threads
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_threads" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_threads" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *AccountQuery) loadPendingOps(ctx context.Context, query *PendingOpQuery, nodes []*Account, init func(*Account), assign func(*Account, *PendingOp)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.PendingOp(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.PendingOpsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_pending_ops
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_pending_ops" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_pending_ops" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *AccountQuery) loadNotifyRules(ctx context.Context, query *NotifyRuleQuery, nodes []*Account, init func(*Account), assign func(*Account, *NotifyRule)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.NotifyRule(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.NotifyRulesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_notify_rules
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_notify_rules" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
